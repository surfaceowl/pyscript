...
desc_fourier = Div(text=open(filename).read(), render_as_text=False, width=plot_width)
...
def create_circle_glyphs(p, color, sources):
    p.circle(
        "x", "y", size=1.0, line_color=color, color=None, source=sources["circleds"]
    )
    p.circle(
        "x", "y", size=5, line_color=color, color=color, source=sources["circle_point"]
    )
    p.line(
        "radius_x",
        "radius_y",
...
def create_centric_plot(
    foos, title="", r=1, y_range=(-2, 2), period=pi / 2, cfoos=None
):
    p = figure(
        title=title,
        tools="",
    ...
    for i, foo in enumerate(foos):
        ...
        if i:
            legend_label = "4sin({c}x)/{c}pi".format(c=i * 2 + 1)
        else:
...
for _k, p in fourier.items():
    p["cplot"], p["csources"] = create_centric_plot(
        p["fs"],
        "Fourier Sine Waves - First 4 Harmonics & Harmonic Circles",
        r=p["f"](period),
        cfoos=p["cfs"],
    )
...
for _k, p in fourier.items():
    p["plot"], p["sources"] = create_plot(
        p["fs"],
        "Fourier Transformation -- (Sum of the first 4 Harmonic Circles)",
        r=p["f"](period),
        cfoos=p["cfs"],
    )

layout = column(
    *[f["cplot"] for f in fourier.values()] + [f["plot"] for f in fourier.values()]
)
